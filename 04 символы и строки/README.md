...
пример: 
Позволāет писатþ строки с вýражениāми
```c#
...
Но естþ недостатки
• Если строки конкатенируĀтсā в øикле – создаетсā много строк
• Как следствие – тратитсā лиúнāā памāтþ и времā на создание строк


Реúение - StringBuilder

```c#
StringBuilder – класс длā конкатенаøии строк
var me = "Эдгар";
...
Созданнаā строка – не менāетсā, совсем
```c#
var plus = "Плюс";
...
String.Length – колиùество char в строке, не символов
```c#
var privet = "Привет"; 
...
ReferenceEquals – сравнение, ùто две ссýлки ссýлаĀтсā на один обüект, а не конкретнýе знаùениā
```c#
var vet = "вет";
var s1 = $"При
{vet}"; var s2 = $"При{vet}";
...
* В CLR (среда вýполнениā C#) – оптимизирована работа со строками
• Суûествует таблиøа – пул интернированиā
• Пул интернированиā – список всех исполþзуемýх строк в программе
• Если строка задана āвнýм образом – автоматиùески помеûаетсā в 
пул интернированиā
• Соответственно – если у двух переменнýх āвно указанý знаùениā 
строк, то они ссýлаĀтсā на одну переменнуĀ
• Если строка создаетсā динамиùески (ввод полþзователем, 
генераøиā функøией) – они не интернируĀтсā → занимаĀт доп. 
памāтþ
• Но строки можно интернироватþ – при помоûи string.Intern – и 
полуùитþ ссýлку на строку из пула
• Если строка уже в пуле – string.Intern возвраûает 
интернированнуĀ строку
```c#
var vet = "вет";
